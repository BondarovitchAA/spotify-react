...
const routes = [
  { url: '/login', title: 'Sign In', icon: 'fa fa-sign-in', authorize: false },
  { url: '/', title: 'Now Playing', icon: 'fa fa-music', authorize: true, default: true },
...
const mapStateToProps = (state, ownProps) => {
  ...
  let currentRoute = routes.find(route => !route.authorize);

  if (state.authorization.isAuthorized) {
    currentRoute = routes.find(route => location.pathname === route.url) || routes.find(route => route.default);
  }
